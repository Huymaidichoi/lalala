                    ...
                    <div>
                        <div class="panel panel-info">
                            <div class="panel-heading">
                                <a href="list_chapter.php?idPost=<?php echo $idPost ?>">HUY</a>
                            </div>
                            <div class="panel-body">
                                <form role="form" method="POST" enctype="multipart/form-data">
                                    ...
                                    <button type="submit" class="btn btn-info" name="txt_sub">Đăng</button>
                                </form>
                                <?php
                                $getdata = new data();
                                if (isset($_POST['txt_sub'])) {
                                    if (empty($_POST['name'])) {
                                        echo "Chưa nhập dữ liệu";
                                    } else {
                                        $nameChapter = $_POST['name'];
                                        $result = $getdata->checkNamechapter($idPost, $nameChapter);
                                        $num = mysqli_num_rows($result);
                                        if ($num >= 1) {
                                            echo "Chapter này đã tồn tại";
                                        } else {
                                            $in_chap = $getdata->addchap($idPost, $_POST['name']);
                                            if ($in_chap) {
                                                echo "<script>alert('Thêm mới thành công')</script>";
                                                echo "<script>window.location:'list_chapter.php?idPost=" . $idPost . "'</script>";
                                            } else {
